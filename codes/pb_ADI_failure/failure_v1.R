# ==============================================================================
# Script: failure_v1.R
# Description: Multi-method comparison study over a continuous 2D parameter grid 
#              (mu* in [1, 3] and epsilon in [0.1, 1.0]). Evaluates empirical 
#              joint and marginal coverage, average joint region area, and 
#              marginal confidence interval widths across Repro (Mahalanobis 
#              depth), Efficient Repro (mu-interest), and PB-ADI.
# ==============================================================================
...
RESULTS_DIR <- file.path(PROJECT_DIR, "results_grid_study_v1")
dir.create(RESULTS_DIR, recursive = TRUE, showWarnings = FALSE)

# ------------------------------------------------------------------------------
# 2. Experimental Parameters & Grid Settings
# ------------------------------------------------------------------------------
...
# Continuous evaluation grid
mu_grid <- c(1.0, 1.5, 2.0, 2.5, 3.0)
ep_grid <- c(0.1, 0.2, 0.5, 1.0)

# Optimization search bounds: Repro and Efficient
mu_search_lower_repro <- -2
mu_search_upper_repro <- 5
sa_search_lower_repro <- 0.1
sa_search_upper_repro <- 5

# Optimization search bounds: PB-ADI
mu_lower_adi <- -2
mu_upper_adi <- 10
sa_lower_adi <- 1e-6
sa_upper_adi <- 10
...
# ------------------------------------------------------------------------------
# 4. Differentially Private Data Generating Process (DGP)
# ------------------------------------------------------------------------------

sdp_vec_repro <- function(data_randomness, privacy_noises, sa, mu, ep) {
...
# ------------------------------------------------------------------------------
# 6. Auxiliary Derivatives & Efficient Direction Computation
# ------------------------------------------------------------------------------

compute_aux_derivatives <- function(mu, sa, aux_dr, aux_pn, ep) {
  h <- h_fd
  synth_center <- sdp_vec_repro(aux_dr, aux_pn, sa, mu, ep)
  
  # Numerical derivative with respect to sigma
  if (sa - h > sa_search_lower_repro && sa + h < sa_search_upper_repro) {
    synth_sa_plus <- sdp_vec_repro(aux_dr, aux_pn, sa + h, mu, ep)
    synth_sa_minus <- sdp_vec_repro(aux_dr, aux_pn, sa - h, mu, ep)
  ...
  if (mu - h > mu_search_lower_repro && mu + h < mu_search_upper_repro) {
    synth_mu_plus <- sdp_vec_repro(aux_dr, aux_pn, sa, mu + h, ep)
    synth_mu_minus <- sdp_vec_repro(aux_dr, aux_pn, sa, mu - h, ep)
...
efficient_direction <- function(mu, sa, interest, aux_dr, aux_pn, cache, ep) {
  ...
  deriv <- compute_aux_derivatives(mu, sa, aux_dr, aux_pn, ep)
...
efficient_depth <- function(synth, mu, sa, interest, aux_dr, aux_pn, cache, ep) {
  direction <- efficient_direction(mu, sa, interest, aux_dr, aux_pn, cache, ep)
...
# ------------------------------------------------------------------------------
# 7. Criterion Scoring & Interval Bisection (Repro Methods)
# ------------------------------------------------------------------------------

score_mu_sa <- function(optim_par, data_randomness, privacy_noises, dp_statistic, depth_type,
                        aux_dr = NULL, aux_pn = NULL, eff_cache = NULL, ep) {
  mu <- optim_par[1]
  sa <- optim_par[2]
  synth <- sdp_vec_repro(data_randomness, privacy_noises, sa, mu, ep)
  synth <- rbind(synth, dp_statistic)
  
  if (depth_type == "efficient_mu") {
    D_synth <- efficient_depth(synth, mu, sa, "mu", aux_dr, aux_pn, eff_cache, ep)
...
sdp_vec_adi <- function(data_randomness, privacy_noises, sa, mu) {
  data <- sa * data_randomness + mu
  t(apply(data, 1, clean_clamp_meanvar)) + privacy_noises
}

score_adi <- function(optim_par, data_randomness, privacy_noises, dp_statistic) {
  mu <- optim_par[1]
  sa <- optim_par[2]
  synth <- sdp_vec_adi(data_randomness, privacy_noises, sa, mu)
  D_synth <- depth.Mahalanobis(dp_statistic, synth)
  -D_synth
}

solve_meanstd_from_clamp <- function(clamped_meanvar, ep) {
...
# ------------------------------------------------------------------------------
# 9. Outer Parameter Grid Loop
# ------------------------------------------------------------------------------

all_master_rows <- list()
start_time_total <- Sys.time()

for (pop_mu in mu_grid) {
  for (current_ep in ep_grid) {
    
    cat("\n============================================================\n")
    cat("RUNNING GRID CELL: mu* =", pop_mu, "| epsilon =", current_ep, "| nSIM =", nSIM, "\n")
    cat("============================================================\n")
    ...
    sim_results <- foreach(
      ...
      raw_data <- rnorm(n, mean = pop_mu, sd = population_sigma)
      ...
      for (depth_type in c("mahalanobis", "efficient_mu")) {
        ...
        d1_rng <- getConfidenceInterval(
          ...
          depth_type, score_mu_sa, aux_dr, aux_pn, this_cache, current_ep
        )
        d2_rng <- getConfidenceInterval(
          ...
          depth_type, score_sa_mu, aux_dr, aux_pn, this_cache, current_ep
        )
        ...
        cov_mu <- as.numeric(pop_mu >= d1_rng[1] && pop_mu <= d1_rng[2])
      ...
      # Evaluate PB-ADI
      ADI_meansd <- solve_meanstd_from_clamp(obs_stats, current_ep)
      ...
      if (any(is.na(ADI_meansd))) {
        ...
        ADI_meansd_pb <- t(
          sapply(seq_len(B_paramboot), function(i) solve_meanstd_from_clamp(noisy_means_vars_new[i, ], current_ep))
        )
        ...
        if (nrow(ADI_meansd_pb) < floor(0.8 * B_paramboot)) {
          ...
          pb_cov_mean <- as.numeric(CI_mean_ends[1] <= pop_mu && CI_mean_ends[2] >= pop_mu)
    ...
    cell_table[, -(1:3)] <- round(cell_table[, -(1:3)], result_digits)
    print(cell_table, row.names = FALSE)
    
    all_master_rows[[length(all_master_rows) + 1]] <- cell_table
    write.csv(cell_table, file.path(RESULTS_DIR, paste0("results_mu", pop_mu, "_ep", current_ep, ".csv")), row.names = FALSE)
  }
}
...
write.csv(master_summary_table, file.path(RESULTS_DIR, "master_grid_summary.csv"), row.names = FALSE)

cat("\n============================================================\n")
cat("Grid Study Complete! Master file saved to:\n", file.path(RESULTS_DIR, "master_grid_summary.csv"), "\n")

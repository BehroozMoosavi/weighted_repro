# ==============================================================================
# Script: exp1_R_aux_sensitivity.R
# Description: Evaluates the sensitivity of the Efficient Repro / Operator-II 
#              confidence intervals to the number of auxiliary draws (R_aux) 
#              used to estimate the efficient direction in Experiment 1.
# ==============================================================================
...
# ------------------------------------------------------------------------------
# 4. R_aux Grid Sensitivity Execution
# ------------------------------------------------------------------------------

PROJECT_DIR <- path.expand("~/R_Simuls/sensitivity_analysis/exp1/R_aux")
...
R_synthetic  <- 200L
lambda_n     <- 1 / log(n)
R_aux_values <- c(50L, 100L, 200L, 400L, 800L)

res_list <- vector("list", length(R_aux_values))
for (j in seq_along(R_aux_values)) {
  R_aux <- R_aux_values[j]
  cat("\n### R_aux =", R_aux, "###\n")
  raw <- run_mc(
    nSIM = nSIM, R_synthetic = R_synthetic, R_aux = R_aux,
    lambda_n = lambda_n, seed_offset = 123L
  )
  write.csv(raw, file.path(PROJECT_DIR, sprintf("R_aux_%d_raw.csv", R_aux)), row.names = FALSE)
  sm <- summarize_mc(raw)
  sm$R_aux <- R_aux
...
write.csv(ALL_RESULTS, file.path(PROJECT_DIR, "R_aux_sensitivity_results.csv"), row.names = FALSE)
...
plot_two_panel(
  ALL_RESULTS,
  x = ALL_RESULTS$R_aux,
  xlab = '"Auxiliary sample size "*italic(R)[plain("aux")]',
  xlab_tex = "Auxiliary sample size $R_{\\mathrm{aux}}$",
  pdf_file = file.path(PROJECT_DIR, "R_aux_sensitivity_publication.pdf"),
  png_file = file.path(PROJECT_DIR, "R_aux_sensitivity_publication.png"),
  nominal = 1 - alpha
)
...
plot_failure_rate(
  ...
  xlab = '"Auxiliary sample size "*italic(R)[plain("aux")]',
  xlab_tex = "Auxiliary sample size $R_{\\mathrm{aux}}$",
  pdf_file = file.path(PROJECT_DIR, "R_aux_failure_rate_publication.pdf"),
  png_file = file.path(PROJECT_DIR, "R_aux_failure_rate_publication.png")
)

writeLines(c(
  sprintf(paste0("\\caption{Sensitivity of the efficient-depth repro-sample intervals to the ",
                 "number of auxiliary draws $R_{\\mathrm{aux}}$ used to estimate the ",
                 "efficient direction. Location--scale normal model with $n = %d$, ",
                 "$(\\mu^*, \\sigma^*) = (%g, %g)$, clamping to $[%g, %g]$, ",
                 "$\\varepsilon = %g$, $R = %d$ repro draws, ",
                 "$\\lambda_n = 1/\\log n = %.4f$, over %d Monte Carlo replicates per grid ",
                 "point. (a) Empirical coverage of the nominal $%g\\%%$ intervals; error bars ",
                 "are $\\pm 1.96$ binomial standard errors and the shaded band is the Monte ",
                 "Carlo tolerance region around the nominal level. (b) Average interval width ",
                 "with $\\pm 1.96$ standard errors. The horizontal axis uses $\\log_2$ ",
                 "spacing. Failed constructions are scored as non-coverage.}"),
          n, population_mu, population_sigma, lower_clamp, upper_clamp, ep,
          R_synthetic, lambda_n, nSIM, 100 * (1 - alpha)),
  "",
...
cat("\n============================================================\n")
cat("R_aux SENSITIVITY COMPLETE\n")
print(ALL_RESULTS)
